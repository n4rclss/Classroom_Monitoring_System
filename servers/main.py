...
def create_response_packet(client_id: str, status: str, message: str) -> bytes:
    ...
    # Message: [TotalMsgLen (4 bytes)][ClientIDLen (1 byte)][ClientID (variable)][ServerResponseData (variable)]
    # Note: This matches the format the load balancer expects when reading from the server.
...
async def handle_connection(reader, writer):
    ...
    try:
        while True:
            ...
            # 4. Process the original client data (expecting JSON login packet)
            response_packet = b''
            try:
                # Attempt to parse as JSON and validate using Pydantic model
                login_request_json = original_client_data.decode('utf-8')
                login_packet = PacketLogin.model_validate_json(login_request_json)
                ...
                if is_authenticated:
                    ...
                    # Check if already logged in (optional, based on requirements)
                    # db.add_active_session(login_packet.username) # Mark as active if needed
                    response_packet = create_response_packet(client_id, "success", "Login successful")
                else:
                    print(f"[*] Login FAILED for user {login_packet.username} (Client ID: {client_id})")
                    response_packet = create_response_packet(client_id, "error", "Invalid credentials or role")
                ...
                response_packet = create_response_packet(client_id, "error", "Internal server error during login")
